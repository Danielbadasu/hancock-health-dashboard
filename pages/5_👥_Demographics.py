...
# ---- SIDEBAR FILTERS (before KPIs so they drive the cards) ----
...
def get_metric(col, county='Hancock', sheet='additional'):
    ...
    if row.empty or col not in row.columns:
        ...
        val = fallback.get(col) if isinstance(fallback, dict) else (fallback[col] if col in fallback.index else None)
...
<div class="kpi-container">
    <div class="kpi-card purple">
        ...
        <div class="kpi-sub">Ohio: {int(pop_o):,} total · Hancock is {round(pop_h/pop_o*100, 2)}% of state</div>
    </div>
...
if "Population Composition" in selected_charts:
    st.markdown('<div class="section-title">Population Composition — Hancock vs Ohio</div>', unsafe_allow_html=True)
    comp_data = pd.DataFrame({
        'Category': ['Under 18', '65 and Older', 'Rural', 'Female', 'Disabled', 'Single-Parent HH'],
        'Hancock County': [
            pct_under18_h, pct_65plus_h, pct_rural_h,
            pct_female_h, pct_disabled_h, pct_singlepar_h
        ],
        'Ohio': [
            pct_under18_o, pct_65plus_o, pct_rural_o,
            pct_female_o, pct_disabled_o, pct_singlepar_o
        ]
    }).dropna()

    cols_to_show = ['Hancock County', 'Ohio'] if show_ohio else ['Hancock County']
    comp_melted = comp_data[['Category'] + cols_to_show].melt(
        id_vars='Category', var_name='Geography', value_name='Percentage'
    )
    fig1 = px.bar(
        comp_melted, x='Category', y='Percentage',
        color='Geography', barmode='group',
        color_discrete_map={'Hancock County': '#a855f7', 'Ohio': '#4ECDC4'},
        labels={'Percentage': 'Percentage (%)', 'Category': ''},
...
if "Age Distribution" in selected_charts:
    ...
    with col1:
        ...
        fig2a = px.pie(
            age_df, values='Percentage', names='Age Group',
    ...
    if show_ohio:
        with col2:
            ...
            st.markdown("**Ohio**")
            fig2b = px.pie(
                age_df_o, values='Percentage', names='Age Group',
                color_discrete_sequence=['#a855f7', '#4ECDC4', '#ffd200'],
                template='plotly_dark', hole=0.45
            )
            fig2b.update_traces(
...
if "Race & Ethnicity" in selected_charts:
    st.markdown('<div class="section-title">Race & Ethnicity — Hancock vs Ohio</div>', unsafe_allow_html=True)
    ...
    cols_to_show = ['Hancock County', 'Ohio'] if show_ohio else ['Hancock County']
    race_melted = race_data[['Group'] + cols_to_show].melt(
        id_vars='Group', var_name='Geography', value_name='Percentage'
    )
    fig3 = px.bar(
        race_melted, x='Group', y='Percentage',
        color='Geography', barmode='group',
        color_discrete_map={'Hancock County': '#a855f7', 'Ohio': '#4ECDC4'},
        labels={'Percentage': 'Percentage (%)', 'Group': ''},
        template='plotly_dark'
    )
    fig3.update_traces(
        hovertemplate='<b>%{x}</b><br>%{fullData.name}: %{y:.1f}%<extra></extra>'
...
# ---- CHART 4: INCOME BY RACE (Ohio context) ----
if "Income by Race" in selected_charts:
    ...
    income_race_cols = {
        'White': 'Household Income (White)',
        'Black': 'Household Income (Black)',
        'Hispanic': 'Household Income (Hispanic)',
        'Asian': 'Household Income (Asian)',
    ...
    if income_race_data:
        ...
        fig4 = px.bar(
            income_race_df, x='Median Income ($)', y='Race/Ethnicity',
            orientation='h',
            color='Median Income ($)',
            color_continuous_scale=['#ff416c', '#ffd200', '#38ef7d'],
        ...
        # Add Hancock overall income line
        if income_h:
            fig4.add_vline(
                x=income_h,
                line_dash='dash',
...
# ---- CHART 5: SINGLE-PARENT HOUSEHOLDS TREND ----
if "Single-Parent Households Trend" in selected_charts:
    st.markdown('<div class="section-title">Children in Single-Parent Households — Trend Over Time</div>', unsafe_allow_html=True)
    ...
    if col:
        ...
        if not show_ohio:
            trend_df = trend_df[trend_df['geography'] == 'Hancock County']

        if not trend_df.empty:
            fig5 = px.scatter(trend_df, x='year', y=col, color='geography',
                              color_discrete_map={'Hancock County': '#a855f7', 'Ohio': '#4ECDC4'},
                              template='plotly_dark')
            for trace in fig5.data:
                geo_data = trend_df[trend_df['geography'] == trace.name]
            ...
            st.info("No single-parent household data in selected year range.")
    else:
        st.info("Single-parent household column not found.")

# ---- CHART 6: DISABILITY & LANGUAGE ----
if "Disability & Language" in selected_charts:
    st.markdown('<div class="section-title">Disability & Language Access — Hancock vs Ohio</div>', unsafe_allow_html=True)
    ...
    cols_to_show = ['Hancock County', 'Ohio'] if show_ohio else ['Hancock County']
    access_melted = access_data[['Indicator'] + cols_to_show].melt(
        id_vars='Indicator', var_name='Geography', value_name='Value'
    )
    fig6 = px.bar(
        access_melted, x='Indicator', y='Value',
        color='Geography', barmode='group',
        color_discrete_map={'Hancock County': '#a855f7', 'Ohio': '#4ECDC4'},
...
export_df = pd.DataFrame({
    ...
    'Hancock County': [
        int(pop_h) if pop_h else None, pct_rural_h, pct_female_h,
        pct_under18_h, pct_65plus_h,
        pct_white_h, pct_black_h, pct_hispanic_h,
        pct_disabled_h, pct_noenglish_h,
        pct_singlepar_h, int(income_h) if income_h else None
    ],
    'Ohio': [
        int(pop_o) if pop_o else None, pct_rural_o, pct_female_o,
        pct_under18_o, pct_65plus_o,
        pct_white_o, pct_black_o, pct_hispanic_o,
        pct_disabled_o, pct_noenglish_o,
        pct_singlepar_o, int(income_o) if income_o else None
    ]
})
csv = export_df.to_csv(index=False).encode('utf-8')
st.download_button(
    label="📥 Download Demographics Data as CSV",
    data=csv,
    file_name=f"hancock_demographics_{selected_year}.csv",
    mime="text/csv"
)
